...
add_image_size( 'pcList_thumbnail', 336, 210, true ); //pc一覧用325px
add_image_size( '200_thumbnail', 200, 150, true ); //pc一覧用200px topランキング部分
add_image_size( '320_thumbnail', 320, 320, true ); //pc動画用
...
function my_custom_popular_posts_html_list( $mostpopular, $instance ){
    ...
    if ( wp_is_mobile() ) { //spかどうか
        foreach( $mostpopular as $popular ) {
            ...
            // $image = wp_get_attachment_image_src( $thumbnail_id, '200_thumbnail' );
            ...
            // if ($tmp_post->post_type === "movingimage_post"){　//spで現在のsingleページが動画postの場合
            ...
            // } else { //spで動画postでない場合
            //     $thumbnail = wp_get_attachment_image_src( $thumbnail_id ,'list_thumbnail');
        ...
        if ($postType->post_type === "movingimage_post"){//pcで現在のsingleページが動画postの場合
               foreach( $mostpopular as $popular ) {
               // URL
               $url = get_the_permalink( $popular->id );
               // ページビュー
               ...
               $author='';
                ...
                $output .=                              '<i class="fa fa-video-camera" aria-hidden="true"></i>';
                $output .=                              '<p>WHATCH MORE</p>';
                $output .=                          '</div>';
            ...
            // $category = get_the_category($mostpopular[0]->id);
            // usort( $category , '_usort_terms_by_ID');//タームID順に取得
            // $category_name = end($category)->cat_name;
            // $category_id = $category[0]->cat_ID;
            // $author = get_the_author_meta('display_name', $mostpopular[0]->uid);
        ...
        //     var_dump($postType);
        //
        //     if ($tmp_post->post_type === "movingimage_post"){//pcで現在のsingleページが動画postの場合
        //         $thumbnail = wp_get_attachment_image_src( $thumbnail_id, '300_thumbnail' );
        //         $mv_cat = get_the_terms($popular->id, 'movingimage_cat');
        //         if( is_array($mv_cat) ){
        //         	usort( $mv_cat , '_usort_terms_by_ID');
    ...
    //
    //
    //     // $movie_type = get_post_type_object('movingimage_post');
    ...
    //           var_dump($mv_cat);
    //         $thumbnail = wp_get_attachment_image_src( $thumbnail_id, '300_thumbnail' );
    //         // $thumbnail = get_the_post_thumbnail( $popular->id ,'300_thumbnail');
    //         $output .= '<li>';
    //         $output .=      '<a href="'. $url .'">';
    //         $output .=          '<div class="content">';
    //         $output .=              '<div class="imgArea" style="background-image: url(' . $thumbnail[0] . ');">' ;
    ...
    //                 // $thumbnail = get_the_post_thumbnail( $popular->id ,'200_thumbnail');
    //             }
    //         $category = get_the_category($popular->id);
    //         usort( $category , '_usort_terms_by_ID');//タームID順に取得
    //         $category_id = $category[0]->cat_ID;
